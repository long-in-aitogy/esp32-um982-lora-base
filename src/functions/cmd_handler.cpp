...
cmd_action_t handleGnssBaseCommand(const std::vector<String> &cmdWords) {
    ...
    if (cmdWords[0] == "SURVEY_IN") {
        ...
        if (!hasExactArgumentCount(cmdWords, 3, "GNSS BASE SURVEY_IN")) {
            return CMD_ACTION_NONE;
        }
        ...
        if (!hasExactArgumentCount(cmdWords, 5, "GNSS BASE FIXED")) {
            return CMD_ACTION_NONE;
        }
...
cmd_action_t handleMqttCommand(const std::vector<String> &cmdWords) {
    if (hasExactArgumentCount(cmdWords, 3, "MQTT" ) && cmdWords[0] == "SET") {
        static constexpr PreferenceMapping stringSettings[] = {
            {"SERVER", "MQTT_SERVER"}, {"USER", "MQTT_USER"},
            {"PASS", "MQTT_PASS"}, {"PUBTPCHEALTH", "TPC_HEALTH"},
            {"PUBTPCRAW", "TPC_RAW_RTCM"}, {"SUBTPCCMD", "TPC_SUB_CMD"},
        };
        const char *key = findPreferenceKey(cmdWords[1], stringSettings,
                                            sizeof(stringSettings) / sizeof(stringSettings[0]));
        if (key != nullptr) {
            return saveStringPreference(key, cmdWords[2]);
        }
        if (cmdWords[1] == "PORT") {
            return saveUShortPreference("MQTT_PORT", static_cast<uint16_t>(cmdWords[2].toInt()));
        }
    }
...
cmd_action_t handleNtripCommand(const std::vector<String> &cmdWords) {
    if (hasExactArgumentCount(cmdWords, 3, "NTRIP") && cmdWords[0] == "SET") {
        static constexpr PreferenceMapping stringSettings[] = {
        ...
        if (cmdWords[1] == "CSTRPORT") {
            return saveUShortPreference("NTRIP_PORT", static_cast<uint16_t>(cmdWords[2].toInt()));
        }
    }
    Serial.println("[MQTT COMMAND DOWNLINK] Lenh sai hoac khong kha dung.");
    return CMD_ACTION_NONE;
